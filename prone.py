...
def quantify_confusion(N, f=lambda x: x * (x - 1) * 0.5, exact=False):
    # N: mixing matrix
    # len(N): número de linhas na matriz N
    if len(N)<=0: return 0
    # Define uma nova função f que está preparada para receber um array
    f = np.vectorize(f, otypes=[np.float])
    # N.shape[0]: número de linhas na matriz N
    # N.shape[1]: número de colunas na matriz N
    # b2: array de tamanho N.shape[1]x1 preenchido com 1's
    b2 = np.ones((N.shape[1], 1), dtype='float')
    # b1: array de tamanho 1xN.shape[0] preenchido com 1's
    b1 = np.ones((1, N.shape[0]), dtype='float')
    # Multiplicação das matrizes b1xN (dimensão = qtd de linhas de b1 e qtd de colunas de N)
    m1 = b1.dot(N)
    # Multiplicação das matrizes Nxb2 (dimensão = qtd de linhas de N e qtd de colunas de b2)
    m2 = N.dot(b2)
    # Multiplicação das matrizes m1xb2 (dimensão = qtd de linhas de m1 e qtd de colunas de b2)
    m = b1.dot(N).dot(b2)
    # f(m2): aplicação de f nos elementos de m2
    # Multiplicação das matrizes b1xf(m2) (dimensão = qtd de linhas de b1 e qtd de colunas de f(m2))
    s1 = (b1.dot(f(m2)))
    # f(m1): aplicação de f nos elementos de m1
    # Multiplicação das matrizes f(m1)xb2 (dimensão = qtd de linhas de f(m1) e qtd de colunas de b2)
    s2 = (f(m1).dot(b2))
    n = f(m)
    I = (b1.dot(f(N)).dot(b2))
...
def get_assors(G, att, vals):
    ...
    # vals = ["Feminino", "Masculino"]
    # len(vals) = 2 
    # C = [[],[]]
    for n,d in G.nodes(data=True):
        # n: identificador do vértice
        # d: dicionários de atributos do vértice 
        C[vals.index(d[att])].append(n)
        # att: gender
        # d[att]: valor do gênero do vértice
        # vals.index(d[att]): posição do gênero na lista
        # C[vals.index(d[att])]: acessa primeira ou segunda lista de C
    q = Q(G, C)
    return {'Q': q[0], 'r': q[1]}
...
def get_all(G,attribute_dict, att, att2=None, print_mixing = False, normalized= False, times  = False , missing_val= 0):
    ...
    if att2==att:
        # attribute_dict[att] = ['Feminino','Masculino']
...
def Q(G, C):
    # C: lista contendo uma lista de ids de vértices femininos e uma lista de ids de vértices masculinos
    # C = [[1234,5236...], [78964,48956...]]
    w = 0.0
    e = 0.0
    ...
    for c in C:
        for i in c:
            for j in c:
                # Soma a quantidade de arestas entre vértices do mesmo tipo (Feminino ou Masculino)
                if i!=j and j in G.neighbors(i): w += 1
                #  Número esperado de arestas entre vértices do mesmo tipo
